...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val resultBoundaries: Boundaries = new Boundaries()

    resultBoundaries.minX = Math.min(a.minX, b.minX)
    resultBoundaries.minY = Math.min(a.minY, b.minY)
    resultBoundaries.maxX = Math.max(a.maxX, b.maxX)
    resultBoundaries.maxY = Math.max(a.maxY, b.maxY)
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    ...
    parBodies.aggregate(matrix)(_ += _, _ combine _)
  }
