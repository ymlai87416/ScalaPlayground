...
class EpidemySuite extends FunSuite {
  ...
  test("prevalence rate"){
    ...
    assert(numInfected == es.SimConfig.population * prevalenceRate,
      "prevalence rate should be 0.01 " + numInfected + " " + (es.SimConfig.population * prevalenceRate)
      )
  }
  ...
  test("life cycle"){
    ...
    assert(infectedPerson.sick == true, "Infected person should become sick after 6 days" + infectedPerson.infected )
